...
pub fn dict_index(options: DictIndexOptions) -> Result<()> {
  ...
  let o_val =
    CString::new(options.output_dir.as_str()).map_err(|e| Error::from_reason(e.to_string()))?;
  let f_flag = CString::new("-f").unwrap();
  let f_val =
    CString::new(options.from_charset.as_str()).map_err(|e| Error::from_reason(e.to_string()))?;
