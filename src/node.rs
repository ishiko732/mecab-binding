...
pub fn nodes_to_vec(node_ptr: *const ffi::mecab_node_t) -> Vec<MecabNode> {
  ...
  while !current.is_null() {
    ...
    if node.stat < 2 {
      let surface = unsafe {
        let bytes = std::slice::from_raw_parts(node.surface as *const u8, node.length as usize);
        String::from_utf8_lossy(bytes).into_owned()
      };
      let feature = unsafe { CStr::from_ptr(node.feature).to_string_lossy().into_owned() };
